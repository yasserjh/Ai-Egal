...
import { LoadingSpinner, DownloadIcon, RegenerateIcon, DiscardIcon, CheckIcon, ErrorIcon, PadlockIcon } from './ui/draggable-card';
...
export const StyleCard: React.FC<StyleCardProps> = ({ styleKey, isSelected, onToggle, lang, isLocked, isSelectionDisabled }) => {
    const styleName = t(lang, `style_${styleKey}_name` as any);
    const styleDesc = t(lang, `style_${styleKey}_desc` as any);
    const isDisabled = isLocked || isSelectionDisabled;

    return (
        <div
            onClick={() => !isDisabled && onToggle(styleKey)}
            className={cn(
                "relative p-4 rounded-2xl border transition-all duration-300 ease-in-out group w-full",
                isDisabled ? "cursor-not-allowed" : "hover:border-neutral-400 hover:shadow-lg",
                isSelected && !isLocked ? "border-transparent bg-white ring-2 ring-[#006C35] shadow-md shadow-[#006C35]/10" : "border-neutral-200/80 bg-white",
                isSelectionDisabled && !isSelected && "opacity-50"
            )}
        >
            <div className={cn("flex items-center justify-between", isLocked && "opacity-40 filter grayscale")}>
                ...
                <div className="relative ltr:ml-4 rtl:mr-4">
                    <div className={cn(
                        ...
                        isSelected && !isLocked ? "border-[#006C35] bg-[#006C35]" : "border-neutral-300 bg-neutral-100 group-hover:border-neutral-400"
                    )}>
                        {isSelected && !isLocked && <CheckIcon className="w-4 h-4 text-white" />}
                    </div>
                </div>
            </div>
             {isLocked && (
                <div className="absolute inset-0 bg-white/50 backdrop-blur-sm flex flex-col items-center justify-center rounded-2xl">
                    <PadlockIcon className="w-8 h-8 text-[#C6A664]" />
                    <p className="mt-2 text-sm font-bold text-center text-neutral-700">{t(lang, 'unlockThisStyle')}</p>
                </div>
            )}
        </div>
    );
